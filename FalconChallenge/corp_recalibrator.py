...
class CORPRecalibrator:
    ...
    def _load_prev_data(self):
        if self.config.prev_data_buffer is not None:
            with open(self.config.prev_data_buffer, "rb") as f:
                self.prev_data_buffer = pickle.load(f)
        else:
            # Load previous sessions' data and pseudo labels
            for i, sess in enumerate(
                self.config.sessions[: self.config.start_session_idx]
            ):
                self.prev_data_buffer[self.config.session_input_layers[i]] = (
                    CORPRecalibrator._load_data(
                        self.config.seed_model_data_dir,
                        sess,
    ...
    def _load_data(data_dir, session, feat_dim, normalize=True):
        ...
        if len(tfrecords) > 0:
            ...
            dataset = dataset.map(
                partial(CORPRecalibrator._parse_proto, feat_dim), num_parallel_calls=4
            )
            for d in dataset:
                d["confidences"] = 1.0
    ...
    def _parse_proto(feat_dim, proto):
        datasetFeatures = {
            "inputFeatures": tf.io.FixedLenSequenceFeature(
                [feat_dim], tf.float32, allow_missing=True
            ),
            "newClassSignal": tf.io.FixedLenSequenceFeature(
    ...
    def _train(self):
        # Compose datasets
        print(
            f"prev_data_buffer: {sum([len(d) for k, d in self.prev_data_buffer.items()])}"
        )
        ...
        output_signature = {
            ...
            "inputFeatures": tf.TensorSpec(
                shape=(None, self.config.feat_dim), dtype=tf.float32
            ),
        ...
        prev_dataset = tf.data.Dataset.from_generator(
            lambda: prev_generator, output_signature=output_signature
        )
        curr_dataset = tf.data.Dataset.from_generator(
            lambda: curr_generator, output_signature=output_signature
        )
        ...
        for data in dataset:
            ...
            try:
                ctc_loss, reg_loss, total_loss, grad_norm = self._train_step(
                    data["inputFeatures"],
                    data["layerIdx"],
                    data["seqClassIDs"],
                    data["nTimeSteps"],
                ...
                if (
                    np.mean(losses[-10:]) < self.config.loss_threshold
                    and steps > self.config.min_train_steps
                ):
                    break
    ...
    def _train_step(
        ...
        with tf.GradientTape() as tape:
            ...
            ctc_loss = tf.compat.v1.nn.ctc_loss_v2(
                sparse_labels,
                logits,
                None,
                time_steps,
            ...
            reg_loss = tf.math.add_n(self.nsd.model.losses) + tf.math.add_n(
                self.nsd.inputLayers[self.config.session_input_layers[sess_idx]].losses
            )
            total_loss = ctc_loss + reg_loss

        # Get trainable variables
        if self.config.freeze_backbone:
            trainables = self.nsd.inputLayers[
                self.config.session_input_layers[sess_idx]
            ].trainable_variables
        else:
            trainables = (
                self.nsd.model.trainable_variables
                + self.nsd.inputLayers[
                    self.config.session_input_layers[sess_idx]
                ].trainable_variables
            )
    ...
    def reset(self, day_idx, decoder):
        ...
        if day_idx != self.curr_day_idx:
            ...
            from_layer = self.nsd.inputLayers[
                self.config.session_input_layers[day_idx - 1]
            ]
            ...
            if len(self.curr_data_buffer) > 0:
                self.prev_data_buffer[
                    self.config.session_input_layers[self.curr_day_idx]
                ] = self.curr_data_buffer.copy()
                self.curr_data_buffer = []
